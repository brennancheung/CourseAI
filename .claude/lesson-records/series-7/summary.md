...
### Module 7.4 (complete)
Three lessons covering next-generation architectures: SDXL (dual encoders, micro-conditioning, refiner--"the U-Net's last stand"), Diffusion Transformers (patchify, adaLN-Zero, transformer scaling recipe), SD3 & Flux (MMDiT joint attention, T5-XXL, flow matching applied at scale--the convergence). The module arc traces the architectural evolution from refined U-Nets (SDXL) through the paradigm shift (DiT) to the current frontier (SD3/Flux). The final lesson delivers the series' emotional payoff: every component of the frontier architecture traces to a lesson the student has already completed. All three lessons have notebooks. The cognitive load progression is BUILD -> STRETCH -> BUILD.

### Series 7 (complete)
Eleven lessons across four modules. From "I understand how Stable Diffusion v1.5 works" (start of 7.1) to "I can read frontier diffusion research and trace every design choice to concepts I built from scratch" (end of 7.4). The student has understanding of: spatial conditioning via ControlNet and IP-Adapter, the score-based perspective unifying DDPM with continuous-time frameworks, flow matching as the training objective shift enabling fewer inference steps, consistency models for 1-4 step generation, the full acceleration taxonomy with composability rules, SDXL as the U-Net ceiling, DiT as the architecture shift from U-Net to transformer, and SD3/Flux as the convergence of transformers + latent diffusion + flow matching + better text encoding. 24 mental models accumulated. The student can look at the SD3 pipeline and explain every component from first principles, tracing each design choice back through 50+ lessons across 7 series.
