...
| Component | Purpose |
|-----------|---------|
| `ZoomableCanvas` | Base Konva canvas with zoom/pan support |
| `MathCanvas` | Canvas optimized for mathematical plots |
| `Grid` | Background grid lines (primitive) |
| `Axis` | X/Y axis with arrows and labels (primitive) |
| `Curve` | Function curve from points array (primitive) |
| `Ball` | Draggable/animated circle (primitive) |

### Score & Playback (Music)
...
### ML/DL Widgets Needed

| Widget | Purpose | Potential Use |
|--------|---------|---------------|
| `ActivationFunctionExplorer` | Compare sigmoid, ReLU, GELU, etc. | Module 1.2: Activation functions |
| `BackpropAnimator` | Animated gradient flow through network | Module 1.3: Backpropagation |
| `OptimizerRace` | Compare SGD, Momentum, Adam on same surface | Module 1.4: Optimizers |
| `OverfittingVisualizer` | Train/val curves diverging | Module 1.5: Regularization |
| `AttentionHeatmap` | Visualize attention weights | Module 2.2: Attention |
| `TokenizerExplorer` | Show BPE/WordPiece tokenization | Module 2.5: Tokenization |
| `SamplingExplorer` | Temperature, top-k, top-p effects | Module 2.5: Sampling |
| `DiffusionStepVisualizer` | Watch noise → image progression | Module 3.2: Diffusion |
